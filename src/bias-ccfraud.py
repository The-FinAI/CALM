...
test_data = pd.read_csv('./bias_data/gpt4_ccfraud_test.csv', sep=',', names=[i for i in range(feature_size)])
test = pd.DataFrame(test_data)
test.columns = mean_list # 表格重新写表头

# method结果读取
# todo 标签需要转换适配各个数据集
res = preres_cc(test.values.tolist(),'flare_ccfraud_desc_write_out_info.json')
